...
class MainApp(App):
    ...
    def random(self):
        ...
        self.help_string.get_screen('Definition App').ids.label_1.text = 'Actual Definition (Missing words will be bolded and underlined)'
        self.help_string.get_screen('Definition App').ids.label_2.text = ''
        
        
    def list_all(self):
        first_half = ''
        second_half = ''
        num_of_definitions = len(self.definitions)
        half = num_of_definitions//2
        counter = 0
        #to try address the label not showeing the full text, split into 2 labels    
        for word in self.definitions:
            definition = self.definitions[word]
            if counter < half:
                first_half += f'[b]{word}[/b]:  {definition}\n\n'
            else:
                second_half += f'[b]{word}[/b]:  {definition}\n\n'

            counter += 1
            
        self.help_string.get_screen('Definition App').ids.label_1.text = first_half
        self.help_string.get_screen('Definition App').ids.label_2.text = second_half
